...
static double __hex_to_double(const char *in)
{
	...
	double dbl;
	unsigned int u;
	unsigned char *pch = reinterpret_cast<unsigned char*>(&dbl);

	IF_FAIL_RETURN_TAG(strlen(in) == sizeof(dbl) * 2, 0, _W, "Type mismatched");

	for (unsigned int i = 0; i < sizeof(dbl); ++i) {
...
bool Json::get(const char *path, const char *key, int64_t *val)
{
	...
	if (vtype == G_TYPE_INT64) {
		...
		*val = static_cast<int64_t>(__hex_to_double(json_node_get_string(node)));
	} else {
		return false;
	}
...
bool Json::get(const char *path, const char *key, double *val)
{
	...
	if (vtype == G_TYPE_DOUBLE) {
		...
		*val = __hex_to_double(json_node_get_string(node));
	} else {
		return false;
	}
