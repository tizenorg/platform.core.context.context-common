...
unsigned int Timer::__instanceCnt = 0;
GMutex Timer::__mutex;
std::map<int, std::pair<ITimerListener*, Timer*>> Timer::__listenerMap;
...
std::string Timer::dowToStr(DayOfWeek dow)
{
	static const char *dowStr[] = {
		DOW_SUN, DOW_MON, DOW_TUE, DOW_WED, DOW_THU, DOW_FRI, DOW_SAT
	};

	if (dow == DayOfWeek::WEEKDAY)	return DOW_WEEKDAY;
	if (dow == DayOfWeek::WEEKEND)	return DOW_WEEKEND;
	if (dow == DayOfWeek::EVERYDAY)	return DOW_EVERYDAY;

	int d = log2(static_cast<double>(dow));
	if (d >= 0 && d < DAY_PER_WEEK)
		return dowStr[d];

	return EMPTY_STRING;
}

DayOfWeek Timer::dowToInt(std::string dow)
{
	if (dow == DOW_SUN)			return DayOfWeek::SUN;
	if (dow == DOW_MON)			return DayOfWeek::MON;
	if (dow == DOW_TUE)			return DayOfWeek::TUE;
	if (dow == DOW_WED)			return DayOfWeek::WED;
	if (dow == DOW_THU)			return DayOfWeek::THU;
	if (dow == DOW_FRI)			return DayOfWeek::FRI;
	if (dow == DOW_SAT)			return DayOfWeek::SAT;
	if (dow == DOW_WEEKDAY)		return DayOfWeek::WEEKDAY;
	if (dow == DOW_WEEKEND)		return DayOfWeek::WEEKEND;
	if (dow == DOW_EVERYDAY)	return DayOfWeek::EVERYDAY;

	return static_cast<DayOfWeek>(0);
}

int Timer::setFor(int interval, ITimerListener *listener)
{
...
int Timer::setAt(int hour, int min, DayOfWeek dow, ITimerListener *listener)
{
	IF_FAIL_RETURN_TAG(
			hour < 24 && hour >= 0 &&
			min < 60 && min >= 0 &&
...
int Timer::__onAlarmExpired(int alarmId, void *userdata)
{
	...
	_D("Stop repeating Timer %d", alarmId);
