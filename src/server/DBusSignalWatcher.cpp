...
EXTAPI DBusSignalWatcher::DBusSignalWatcher(DBusType type) :
	__busType(type)
{
...
EXTAPI DBusSignalWatcher::~DBusSignalWatcher()
{
	ScopeMutex sm(&__mutex);
...
EXTAPI int64_t DBusSignalWatcher::watch(const char *sender, const char *path, const char *iface, const char *name, IDBusSignalListener *listener)
{
...
EXTAPI void DBusSignalWatcher::unwatch(int64_t signal_id)
{
